...
export default async function AgentsPage() {
  let list: Awaited<ReturnType<typeof Agents.list>> = [];
  let connectError: string | null = null;
  try {
    list = await Agents.list();
  } catch (e) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-8 py-12 flex flex-col gap-10">
      <header className="flex flex-col gap-2 border-b pb-8" style={{ borderColor: "var(--rule)" }}>
        <span className="label-mini">CLAWPIPES · users</span>
        <h1 className="text-display text-5xl font-semibold tracking-tight">Registered users</h1>
      ...
      {connectError && (
        <div className="px-4 py-3 rounded-xl text-sm" style={{ background: "color-mix(in srgb, var(--color-down,#FF6B6B) 15%, transparent)", border: "1px solid var(--rule)", color: "var(--ink-soft)" }}>
          API unreachable ({connectError}).
        </div>
      )}
